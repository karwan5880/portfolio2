...
export class TextRenderer {
  ...
  createTextTexture(text, options = {}) {
    const textureData = this.renderText(text, options)
...
export function createDefaultTimeline() {
  ...
  // Bright orange-red neon color like Chongqing neon signs
  const neonOrangeRed = [1.0, 0.3, 0.1] // Bright orange-red

  // Chinese New Year message sequence with optimized settings
  const chineseOptions = {
    ...
    letterSpacing: 0.01, // Extremely minimal letter spacing - less than 1 drone width
    fontSize: 34, // Maximum size to fully utilize available space
    scaleY: 1.1, // Slightly stretch height to open up internal spaces
  }

  // Refined New Year message sequence
  timeline.addMessage(55.0, 5.0, '新年快乐', neonOrangeRed, chineseOptions) // 55-60s: Happy New Year (5s)
  timeline.addMessage(61.0, 6.0, '家承运昌', neonOrangeRed, chineseOptions) // 61-67s: Family inherits prosperity (6s)
  timeline.addMessage(68.0, 7.0, '身安心安', neonOrangeRed, chineseOptions) // 68-75s: Health & peace of mind (7s)
  timeline.addMessage(76.0, 999.0, '艺臻恬然', neonOrangeRed, chineseOptions) // 76s+: Art reaches tranquil perfection (10s+ forever)
