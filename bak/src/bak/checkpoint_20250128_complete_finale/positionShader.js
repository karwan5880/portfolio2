...
export const dronePositionShader = /*glsl*/ `
    ...
    vec3 calculateFinalePosition(float time, float droneId, vec3 rectanglePos) {
        float finaleStartTime = 120.0; // Start finale after last text (111s + 9s buffer)
        ...
        float finaleDuration = 8.0; // 8 seconds to fly towards camera
        ...
        // Smooth movement towards camera with acceleration
        float progress = finaleTime / finaleDuration;
        float acceleratedProgress = progress * progress; // Quadratic acceleration
        
        return mix(rectanglePos, finalTarget, smoothstep(0.0, 1.0, acceleratedProgress));
    }
