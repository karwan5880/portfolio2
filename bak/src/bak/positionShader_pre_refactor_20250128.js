...
export const dronePositionShader = /*glsl*/ `
    ...
    vec3 calculateFormationPosition(int formationState, float time, float row, float col, float droneId, vec3 splitPos) {
        ...
        if (formationState == 4) {
            ...
            // VERY FAST SPEED MOVEMENT SYSTEM
            float CONSTANT_SPEED = 200.0; // Units per second - VERY FAST formation
            ...
            float basePauseDuration = 0.1; // Very short pause between axes
            float randomPauseVariation = random(randomSeed + 2000.0) * 0.1; // Minimal random pause
            float axisPauseDuration = basePauseDuration + randomPauseVariation; // 0.1-0.2s very short pauses
            ...
            float maxDelay = 1.0; // REDUCED from 5.0 to 1.0 - for faster debugging
            float distanceBasedDelay = maxDelay * (1.0 - distanceRatio); // Invert: far=0, close=maxDelay
            
            // NO WAVE DELAY: All rows start together for faster formation
            float waveDelay = 0.0; // All drones start at the same time
            
            float colMicroStagger = col * 0.02; // Reduced from 0.05s to 0.02s
            
            // FUN RANDOM TIMING: Add uncertainty for all 4096 drones! (REDUCED FOR DEBUGGING)
            float randomDelay = random(randomSeed) * 2.0; // 0-2 seconds random delay (was 8.0)
