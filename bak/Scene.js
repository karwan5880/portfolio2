...
function Scene({ speedMultiplier = 1.0 }) {
  ...
  useFrame((state) => {
    ...
    const finalPos = { x: 0, y: 100, z: 600 } // Closer and lower, looking up
...
function PostProcessing({ speedMultiplier = 1.0 }) {
  ...
  useFrame((state) => {
    ...
    const columnRevealEnd = 15.0 // 5-15s: column reveal with dim light (NO BLOOM) - EXTENDED
    const flightStartTime = 15.0 // 15s+: drones start flying row by row - DELAYED
