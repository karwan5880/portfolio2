...
const Experience = React.forwardRef(function Experience({ lightTracker, sunRef }, ref) {
  const initialRotationSpeed = 0.25 // Slower Earth rotation
  ...
  useEffect(() => {
    if (isInitialDelayOver) {
      const accelerationInterval = setInterval(() => {
        setPlanetRotationSpeed((prev) => Math.min(prev + 0.005, 0.8)) // Lower max speed
  ...
  useFrame((state, delta) => {
    ...
    const angle = time * 0.2 + sunInitialAngle // Faster sun orbit
    const orbitRadius = 60

    // Add some Y variation for more realistic sun movement
    const sunY = Math.sin(angle * 0.3) * 15
    sunPosition.set(Math.sin(angle) * orbitRadius, sunY, Math.cos(angle) * orbitRadius)
  ...
  return (
    <>
      ...
      <directionalLight ref={directionalLightRef} intensity={1.0} /> {/* Reduced intensity */}
      <Stars ref={starsRef} radius={500} depth={80} count={15000} factor={10} saturation={0} fade speed={1.5} />
      <AsteroidBelt />
      <group ref={planetGroupRef} rotation={[-0.02, 0, 0]}>
        <PhotorealisticPlanet ref={solidPlanetRef} />
        {locations.map((loc) => (
          <Pin key={loc.name} status={loc.status} occluderRef={solidPlanetRef} position={latLonToVector3(loc.coords[0], loc.coords[1], 2.02)} tooltip={`${loc.name} - ${loc.subtitle}`} />
        ))}
      </group>
      {/* Atmosphere outside the rotating group for proper sun position tracking */}
