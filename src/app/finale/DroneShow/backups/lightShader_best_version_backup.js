...
export const droneRenderShaderVS = /*glsl*/ `
    ...
    vec3 getRainbowColumnColor(float droneId, float time, float revealFactor, vec3 worldPosition) {
        ...
        // RECTANGLE FORMATION: Text display or random colors
        if (time >= 52.0) {
            // TEXT DISPLAY SYSTEM: Check if text is active
            if (uTextActive > 0.5) {
                // Calculate texture coordinates for this drone
                ...
                // vec2 textureCoord = vec2(row / 127.0, col / 31.0);
                ...
                if (row >= 64.0 && col <= 15.0) {
                    ...
                    // Bottom-left plane → texture LEFT half, BOTTOM half
                    textureX = (63.0 - row);           // row 63→0 becomes textureX 0→63
                    textureY = 16.0 + (15.0 - col);   // col 15→0 becomes textureY 16→31
                } else {
                    // Bottom-right plane → texture RIGHT half, BOTTOM half
                    textureX = 64.0 + (63.0 - row);   // row 63→0 becomes textureX 64→127
                    textureY = 31.0 - (col - 16.0);   // col 16→31 becomes textureY 31→16
                }
                
                // Convert to normalized coordinates
                float u = textureX / 127.0;
                float v = textureY / 31.0;
                ...
                if (textMask > 0.5) {
                    return uTextColor; // Text pixels - bright
                } else {
                    return vec3(0.02, 0.02, 0.02); // Background drones - very dim/almost off
                }
            } else {
                // DISCO MODE: Random colors change every 2 seconds (FALLBACK)
