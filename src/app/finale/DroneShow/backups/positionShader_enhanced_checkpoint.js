// CHECKPOINT: Enhanced drone show with improved split formation and variable speeds
// Saved on: Current date
// This version includes:
// - Ultra-wide split formation (-500 to +500 X range)
// - Elevated random heights (400-700) for split formation
// - Variable speed system (Y=slow, X=medium, Z=fast)
// - Back-to-front wave pattern (1s per row)
// - Distance-based timing (reduced to 5s max)
// - Random timing variations (0-8s delay, 0.7x-1.3x speed)
// - Slow movement for testing (20 units/sec base)
...
export const dronePositionShader = /*glsl*/ `
    ...
    vec3 calculateFormationPosition(int formationState, float time, float row, float col, float droneId, vec3 splitPos) {
        ...
        if (formationState == 4) {
            ...
            // FASTER SPEED MOVEMENT SYSTEM
            float CONSTANT_SPEED = 80.0; // Units per second - INCREASED for faster formation
            ...
            float basePauseDuration = 0.5; // Base 0.5 second pause between axes - REDUCED
            float randomPauseVariation = random(randomSeed + 2000.0) * 0.5; // 0-0.5s additional pause - REDUCED
            float axisPauseDuration = basePauseDuration + randomPauseVariation; // 0.5-1.0s random pauses
            ...
            // Distance-based delay: REDUCED so wave pattern is more visible
            float distanceRatio = totalDistance / maxEstimatedDistance;
            float maxDelay = 5.0; // REDUCED from 20.0 to 5.0 - let wave pattern dominate
            float distanceBasedDelay = maxDelay * (1.0 - distanceRatio); // Invert: far=0, close=maxDelay
            
            // SLOW WAVE FOR TESTING: Back rows start first (back to front)
            float waveDelay = 0.0;
            if (row >= 96.0) {
                waveDelay = (127.0 - row) * 1.0; // 0-31s wave (row 127 starts first - back row)
            } else if (row >= 64.0) {
                waveDelay = (95.0 - row) * 1.0; // 0-31s wave (row 95 starts first - back row)
            } else if (row >= 32.0) {
                waveDelay = (row - 32.0) * 1.0; // 0-31s wave (row 32 starts first - back row)
            } else {
                waveDelay = row * 1.0; // 0-31s wave (row 0 starts first - back row)
            }
            
            float colMicroStagger = col * 0.02; // Reduced from 0.05s to 0.02s
            
            // FUN RANDOM TIMING: Add uncertainty for all 4096 drones! (RESTORED)
            ...
            // VARIABLE SPEED SYSTEM: Different speeds for each axis
            float ySpeed = randomizedSpeed * 0.5; // Y = SLOW (50% of base speed)
            float xSpeed = randomizedSpeed * 1.0; // X = MEDIUM (100% of base speed) 
            float zSpeed = randomizedSpeed * 2.0; // Z = FAST (200% of base speed)
            
            // Calculate time needed for each axis with different speeds
            float yMoveDuration = yDistance / ySpeed;   // Slowest movement
            float xMoveDuration = xDistance / xSpeed;   // Medium movement
            float zMoveDuration = zDistance / zSpeed;   // Fastest movement
    ...
    vec3 calculatePosition(float time, float row, float col, float droneId, vec3 groundPos) {
        ...
        if (state >= 3) {
            ...
            if (isLeftGroup) {
                // EXTRA WIDE LEFT GROUP: spans -500 to -100 (center at -300)
                float newX = -500.0 + col * (400.0 / 15.0); // -500 to -100 over 16 columns (0-15)
                // ELEVATED RANDOM Y: Each drone gets random height between 400-700
                float randomY = 400.0 + random(droneId * 0.001 + 5000.0) * 300.0; // 400-700 random height
                targetSplitPos = vec3(newX, randomY, groundPos.z - 600.0);
            } else {
                // EXTRA WIDE RIGHT GROUP: spans +100 to +500 (center at +300)
                float newX = 100.0 + (col - 16.0) * (400.0 / 15.0); // +100 to +500 over 16 columns (16-31)
                // ELEVATED RANDOM Y: Each drone gets random height between 400-700
                float randomY = 400.0 + random(droneId * 0.001 + 5000.0) * 300.0; // 400-700 random height
                targetSplitPos = vec3(newX, randomY, groundPos.z - 600.0);
            }
