...
// Pattern rendering class for colorful geometric patterns
export class PatternRenderer {
  constructor() {
    ...
    this.ctx.imageSmoothingEnabled = false // Sharp pixels for patterns
  }

  // Create rainbow gradient pattern
  createRainbowPattern(time = 0) {
    const imageData = this.ctx.createImageData(128, 32)
    ...
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 128; x++) {
        ...
        const hue = ((x / 128) * 360 + time * 50) % 360
        const saturation = 1.0
        const lightness = 0.5 + 0.3 * Math.sin((y / 32) * Math.PI * 2 + time * 2)

        const rgb = this.hslToRgb(hue / 360, saturation, lightness)

        data[index] = rgb[0] * 255 // R
        data[index + 1] = rgb[1] * 255 // G
        data[index + 2] = rgb[2] * 255 // B
        data[index + 3] = 255 // A
      }
    }

    this.ctx.putImageData(imageData, 0, 0)
    return this.processCanvasToTexture()
  }

  // Create spiral pattern
  createSpiralPattern(time = 0) {
    const imageData = this.ctx.createImageData(128, 32)
    const data = imageData.data
    ...
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 128; x++) {
        ...
        // Spiral calculation
        const spiral = (angle + distance * 0.2 + time * 3) % (Math.PI * 2)
        const intensity = (Math.sin(spiral) + 1) * 0.5

        // Color based on position and time
        const hue = (((angle + Math.PI) / (Math.PI * 2)) * 360 + time * 30) % 360
        const rgb = this.hslToRgb(hue / 360, 1.0, intensity * 0.7)
  ...
  createWavePattern(time = 0) {
    ...
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 128; x++) {
        ...
        const wave1 = Math.sin(x * 0.1 + time * 2) * Math.sin(y * 0.2 + time * 1.5)
        const wave2 = Math.sin(x * 0.15 + y * 0.1 + time * 3)
        const wave3 = Math.sin((x - 64) * 0.08 + (y - 16) * 0.08 + time * 2.5)

        const combined = (wave1 + wave2 + wave3) / 3
        const intensity = (combined + 1) * 0.5

        // Color cycling
        const hue = (time * 60 + x * 2 + y * 4) % 360
        const rgb = this.hslToRgb(hue / 360, 0.8, intensity * 0.8)
  ...
  // Create geometric mandala pattern
  createMandalaPattern(time = 0) {
    const imageData = this.ctx.createImageData(128, 32)
    const data = imageData.data
    ...
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 128; x++) {
        ...
        // Mandala pattern with multiple symmetries
        const symmetry = 8
        const normalizedAngle = ((angle + Math.PI) / (Math.PI * 2)) * symmetry
        const symmetricAngle = (normalizedAngle % 1) * Math.PI * 2

        const pattern = Math.sin(distance * 0.3 + time) * Math.cos(symmetricAngle * 3 + time * 2) * Math.sin(distance * 0.1 + symmetricAngle + time * 1.5)

        const intensity = (pattern + 1) * 0.5

        // Rainbow colors based on angle and distance
        const hue = (((angle + Math.PI) / (Math.PI * 2)) * 360 + distance * 10 + time * 40) % 360
        const rgb = this.hslToRgb(hue / 360, 1.0, intensity * 0.7)
  ...
  // Convert canvas to texture data
  processCanvasToTexture() {
    const imageData = this.ctx.getImageData(0, 0, 128, 32)
    const textureData = new Float32Array(128 * 32 * 4)
    ...
    for (let i = 0; i < imageData.data.length; i += 4) {
      ...
      textureData[textureIndex + 3] = a
    }

    return textureData
  }

  // Create Nezuko-inspired pattern
  createNezukoPattern(time = 0) {
    const imageData = this.ctx.createImageData(128, 32)
    const data = imageData.data

    // Nezuko color palette (pink/orange theme)
    const nezukoColors = [
    ...
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 128; x++) {
        ...
        // Create a flowing pattern inspired by Nezuko's colors
        const wave1 = Math.sin(x * 0.05 + time) * Math.cos(y * 0.1 + time * 0.5)
        const wave2 = Math.cos(x * 0.08 + y * 0.05 + time * 1.2)
        const combined = (wave1 + wave2) * 0.5 + 0.5

        // Use Nezuko's color palette with safe bounds
        const colorIndex = Math.max(0, Math.min(Math.floor(combined * nezukoColors.length), nezukoColors.length - 1))
        const color = nezukoColors[colorIndex]

        data[index] = color[0] || 255
        data[index + 1] = color[1] || 182
        data[index + 2] = color[2] || 193
        data[index + 3] = 255
      }
    }
  ...
  // Create Pikachu-inspired pattern
  createPikachuPattern(time = 0) {
    const imageData = this.ctx.createImageData(128, 32)
    const data = imageData.data

    // Pikachu colors (yellow/electric theme)
    const pikachuColors = [
    ...
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 128; x++) {
        ...
        const lightning = Math.sin(x * 0.2 + time * 3) * Math.cos(y * 0.3 + time * 2)
        const spark = Math.random() > 0.95 ? 1 : 0 // Random sparks
        const combined = Math.max(lightning * 0.5 + 0.5, spark)

        const colorIndex = Math.max(0, Math.min(Math.floor(combined * pikachuColors.length), pikachuColors.length - 1))
        const color = pikachuColors[colorIndex]
  ...
  // Create Digimon-inspired pattern
  createDigimonPattern(time = 0) {
    const imageData = this.ctx.createImageData(128, 32)
    const data = imageData.data

    // Digital/cyber colors
    const digimonColors = [
      [0, 255, 255], // Cyan
    ...
    for (let y = 0; y < 32; y++) {
      for (let x = 0; x < 128; x++) {
        ...
        const gridX = Math.sin(x * 0.3 + time * 2) > 0.7 ? 1 : 0
        const gridY = Math.sin(y * 0.5 + time * 1.5) > 0.7 ? 1 : 0
        const digital = gridX || gridY ? 1 : Math.sin((x + y) * 0.1 + time) * 0.5 + 0.5

        const colorIndex = Math.max(0, Math.min(Math.floor(digital * digimonColors.length), digimonColors.length - 1))
        const color = digimonColors[colorIndex]
  ...
  // Create THREE.js texture from pattern
  createPatternTexture(patternType, time = 0) {
    let textureData

    switch (patternType) {
      case 'rainbow':
        textureData = this.createRainbowPattern(time)
        break
      case 'spiral':
        textureData = this.createSpiralPattern(time)
        break
      case 'wave':
        textureData = this.createWavePattern(time)
        break
      case 'mandala':
        textureData = this.createMandalaPattern(time)
        break
      case 'nezuko':
        textureData = this.createNezukoPattern(time)
        break
      case 'pikachu':
        textureData = this.createPikachuPattern(time)
        break
      case 'digimon':
        textureData = this.createDigimonPattern(time)
        break
      default:
        textureData = this.createRainbowPattern(time)
    }

    const texture = new THREE.DataTexture(textureData, 128, 32, THREE.RGBAFormat, THREE.FloatType)
...
export function createDefaultTimeline() {
  ...
  timeline.addMessage(90.0, 10.0, '恭喜發財', emotionalBlueWhite, chineseOptions) // 90-100s: Happy New Year (10s with fade-out)
...
// Create anime pattern timeline
export function createAnimeTimeline() {
  const timeline = new TextTimeline()

  // Anime pattern timeline
  timeline.addMessage(90.0, 10.0, 'nezuko', [1, 1, 1], { patternType: 'nezuko' })
  timeline.addMessage(101.0, 10.0, 'pikachu', [1, 1, 1], { patternType: 'pikachu' })
  timeline.addMessage(112.0, 10.0, 'digimon', [1, 1, 1], { patternType: 'digimon' })
  timeline.addMessage(123.0, 999.0, 'rainbow', [1, 1, 1], { patternType: 'rainbow' })

  return timeline
}

// Create pattern-based timeline to replace text messages
export function createPatternTimeline() {
  const timeline = new TextTimeline()

  // Pattern timeline - using 'text' field to store pattern type
  timeline.addMessage(90.0, 10.0, 'rainbow', [1, 1, 1], { patternType: 'rainbow' })
  timeline.addMessage(101.0, 10.0, 'spiral', [1, 1, 1], { patternType: 'spiral' })
  timeline.addMessage(112.0, 10.0, 'wave', [1, 1, 1], { patternType: 'wave' })
  timeline.addMessage(123.0, 999.0, 'mandala', [1, 1, 1], { patternType: 'mandala' })
