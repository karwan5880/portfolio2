...
export function DroneShow() {
  ...
  // Handle finale text sequential reveal with 8-second pause
  useEffect(() => {
    // Show first line after 145 seconds (8 seconds after finale starts at 137s - 8s pause)
    const firstLineTimer = setTimeout(
      () => {
        ...
        // Show home button after all animations complete (5 seconds after third line)
        const homeButtonTimer = setTimeout(() => {
        ...
        return () => {
          ...
          clearTimeout(homeButtonTimer)
        }
      },
      (145 * 1000) / SHOW_SPEED_MULTIPLIER
    ) // 145 seconds adjusted for speed (8 seconds after finale starts)
