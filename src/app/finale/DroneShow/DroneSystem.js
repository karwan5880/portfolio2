...
import { PatternRenderer, TextRenderer, createAnimeTimeline, createDefaultTimeline, createPatternTimeline } from './TextRenderer.js'
...
export function DroneSystem({ scale = 1.0, pulseFrequency = 5.0, speedMultiplier = 1.0 }) {
  ...
  const patternRenderer = useMemo(() => new PatternRenderer(), [])
  const textTimeline = useMemo(() => createAnimeTimeline(), []) // Changed to anime timeline
  ...
  useFrame((state) => {
    ...
    if (isTextActive && currentMessage) {
      // Check if this is a pattern or text message
      const isPattern = currentMessage.options && currentMessage.options.patternType

      if (isPattern) {
        // Create animated pattern texture
        const patternType = currentMessage.options.patternType

        // Always update pattern for animation - dispose previous frame
        if (currentTextTexture && currentTextTexture.texture) {
          currentTextTexture.texture.dispose()
        }

        // Create new animated pattern texture with current time
        const newTexture = patternRenderer.createPatternTexture(patternType, time)
        ...
        // For patterns, we use the texture colors directly
        lightMaterial.uniforms.uTextActive.value = 1.0
        lightMaterial.uniforms.uTextColor.value.set(1, 1, 1) // White to preserve pattern colors
        bodyMaterial.uniforms.uTextActive.value = 1.0
        bodyMaterial.uniforms.uTextColor.value.set(1, 1, 1)
      } else {
