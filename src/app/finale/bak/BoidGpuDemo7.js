...
// --- START OF SIMULATION SHADER CHANGES ---
const SimulationMaterial = shaderMaterial(
  {
    ...
    uSearchRadius: 1.0, // --- NEW: Add search radius uniform, default to 1.0 (3x3 grid)
  },
  /*glsl*/ `varying vec2 vUv;void main(){vUv=uv;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`,
  /*glsl*/ `varying vec2 vUv;uniform sampler2D uPositionTexture;uniform sampler2D uTargetPositionTexture;uniform sampler2D uDroneDataTexture;uniform float uTime;uniform float uSwitchTime;uniform vec2 uTexelSize;uniform float uRepulsionRadius;uniform float uRepulsionStrength;
    
    uniform float uSearchRadius; // --- NEW: Make the shader aware of the search radius
    ...
    void main(){
      ...
      // --- UPDATED: The loop bounds are now controlled by our new uniform ---
...
// --- END OF SIMULATION SHADER CHANGES ---
...
function BoidsSimulation() {
  ...
  // --- UPDATED: Add a control for the search radius ---
  const { repulsionRadius, repulsionStrength, searchRadius } = useControls('Drone Spacing', {
    ...
    searchRadius: { value: 1, min: 1, max: 5, step: 1 }, // 1=3x3, 2=5x5, etc.
  })
  ...
  useFrame(({ gl, clock }) => {
    ...
    // --- UPDATED: Pass all leva controls to the shader each frame ---
