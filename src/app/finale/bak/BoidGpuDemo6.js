...
const SimulationMaterial = shaderMaterial(
  ...
  void main(){
    ...
    vec3 neighbor_left=texture2D(uPositionTexture,vUv-vec2(uTexelSize.x,0.0)).xyz;
    vec3 neighbor_right=texture2D(uPositionTexture,vUv+vec2(uTexelSize.x,0.0)).xyz;
    vec3 neighbor_top=texture2D(uPositionTexture,vUv+vec2(0.0,uTexelSize.y)).xyz;
    vec3 neighbor_bottom=texture2D(uPositionTexture,vUv-vec2(0.0,uTexelSize.y)).xyz;
    vec3 diff_left=intendedPos-neighbor_left;
    if(length(diff_left)<uRepulsionRadius){
        repulsionVector+=normalize(diff_left)*(uRepulsionRadius-length(diff_left));
    }
    vec3 diff_right=intendedPos-neighbor_right;
    if(length(diff_right)<uRepulsionRadius){
        repulsionVector+=normalize(diff_right)*(uRepulsionRadius-length(diff_right));
    }
    vec3 diff_top=intendedPos-neighbor_top;
    if(length(diff_top)<uRepulsionRadius){
        repulsionVector+=normalize(diff_top)*(uRepulsionRadius-length(diff_top));
    }
    vec3 diff_bottom=intendedPos-neighbor_bottom;
    if(length(diff_bottom)<uRepulsionRadius){
        repulsionVector+=normalize(diff_bottom)*(uRepulsionRadius-length(diff_bottom));
    }
...
function BoidsSimulation() {
  ...
  //   //
  //   const { initialPositionTexture, targetPositionTexture, particleGeometry } = useMemo(() => {
  //     // --- DATA GENERATION ---

  //     const linePositionData = new Float32Array(NUM_PARTICLES * 4);
  //     const rectanglePositionData = new Float32Array(NUM_PARTICLES * 4);
  //     const droneSpacing = 0.2 // The distance between drones in the starting line
  //     // 1. Generate the 'Straight Line' starting positions
  //     for (let i = 0; i < NUM_PARTICLES; i++) {
  //         const k = i * 4;
  //         // Simple horizontal line, centered at x=0, and placed low on the y-axis
  //         linePositionData[k + 0] = (i - NUM_PARTICLES / 2) * droneSpacing;
  //         linePositionData[k + 1] = -10.0;
  //         linePositionData[k + 2] = 0.0;
  //         linePositionData[k + 3] = 1.0;
  //     }
  //     const gridSpacing = 0.7; // The distance between drones in the final rectangle
  //     // 2. Generate the '20x20 Rectangle' target positions
  //     for (let i = 0; i < SIZE; i++) { // Corresponds to X axis
  //       for (let j = 0; j < SIZE; j++) { // Corresponds to Z axis
  //         const k = (i * SIZE + j) * 4;
  //         // Position drones in a grid on the XZ plane, centered at the origin
  //         rectanglePositionData[k + 0] = (i - SIZE / 2) * gridSpacing;
  //         // Place the grid high up in the "sky"
  //         rectanglePositionData[k + 1] = 10.0;
  //         rectanglePositionData[k + 2] = (j - SIZE / 2) * gridSpacing;
  //         rectanglePositionData[k + 3] = 1.0;
  //       }
  //     }
  //     // Create the DataTextures that the GPU will use
  //     const initialPosTex = new THREE.DataTexture(linePositionData, SIZE, SIZE, THREE.RGBAFormat, THREE.FloatType);
  //     initialPosTex.needsUpdate = true;
  //     const targetPosTex = new THREE.DataTexture(rectanglePositionData, SIZE, SIZE, THREE.RGBAFormat, THREE.FloatType);
  //     targetPosTex.needsUpdate = true;
  //     // Create the geometry for the points. We only need UVs to look up the position data.
  //     const geometry = new THREE.BufferGeometry();
  ...
  //     geometry.setAttribute('uv', new THREE.BufferAttribute(uvs, 2));
  //     return {
  //         initialPositionTexture: initialPosTex,
  //         targetPositionTexture: targetPosTex,
