...
const SIZE = 256 // This is our particle grid size (256x256 = 65,536 particles)

export function useBoidData() {
  const gltf = useGLTF('/monkey.glb')
  // Single state variable to hold the final, complete data object.
  const [data, setData] = useState(null)

  useEffect(() => {
    // Only run if the GLTF model is ready
    if (!gltf.nodes.mesh_0) return

    // --- START OF THE CHEAT ---

    // 1. Create an offscreen 2D canvas to draw our text on.
    const canvas = document.createElement('canvas')
    const ctx = canvas.getContext('2d')
    const canvasWidth = 160 // We can tweak these dimensions
    const canvasHeight = 40
    canvas.width = canvasWidth
    canvas.height = canvasHeight

    // 2. Configure and draw the text. No external files needed!
    //    'Arial' is a web-safe font that will always be available.
    ctx.fillStyle = 'white'
    ...
    // 3. Read the pixel data from the canvas.
    const imageData = ctx.getImageData(0, 0, canvasWidth, canvasHeight).data

    // 4. Create an array to hold the coordinates of just the text pixels.
    const textCoordinates = []
    for (let y = 0; y < canvasHeight; y++) {
      for (let x = 0; x < canvasWidth; x++) {
        // The imageData is a flat array: [R, G, B, A, R, G, B, A, ...]
        // We check the Alpha channel (the 4th value) to see if the pixel is drawn.
        const alpha = imageData[(y * canvasWidth + x) * 4 + 3]
        if (alpha > 0) {
          // We found a pixel belonging to the text. Store its coordinates.
          // We center the coordinates around (0,0) and scale them up.
          textCoordinates.push({
            x: (x - canvasWidth / 2) * 0.1,
            y: (canvasHeight / 2 - y) * 0.1, // Invert Y-axis for 3D space
          })
        }
      }
    }

    // 5. Create our final data textures for the GPU.
    const textPosData = new Float32Array(SIZE * SIZE * 4)
    const textColorData = new Float32Array(SIZE * SIZE * 4)
    const textColor = new THREE.Color('#fafad2') // Gold color

    for (let i = 0; i < SIZE * SIZE; i++) {
      // Pick a random coordinate from our list of text pixels
      const coord = textCoordinates[i % textCoordinates.length]

      const k = i * 4
      textPosData[k + 0] = coord.x
      textPosData[k + 1] = coord.y
      textPosData[k + 2] = 0 // Keep it flat on the Z-axis
    ...
    // --- All the original logic for other shapes remains the same ---
    ...
    const allFormations = [
      ...
      textFormation, // Add our new, guaranteed-to-work text formation
    ]
