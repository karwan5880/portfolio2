    ...
    >
      <div className="flex items-center space-x-1 bg-black/80 backdrop-blur-xl rounded-full px-2 py-2 border border-white/10">
        {navItems.map((item) => (
          <motion.button
            key={item.id}
            className={`magnetic relative px-4 py-2 rounded-full text-sm font-medium transition-colors ${activeSection === item.id ? 'text-black' : 'text-white hover:text-gray-300'}`}
  ...
  return (
    <section ref={containerRef} className="relative h-screen flex items-center justify-center overflow-hidden bg-black">
      ...
      <motion.div className="text-center z-10" style={{ y, opacity, scale }}>
        ...
        {/* Morphing Title */}
        <motion.div
          className="text-2xl md:text-3xl font-light text-gray-300 mb-12 overflow-hidden"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <motion.button
          className="magnetic group relative px-8 py-4 bg-transparent border-2 border-white text-white font-medium rounded-full overflow-hidden"
          ...
          {projects.map((project, index) => (
            ...
            >
              ...
              <motion.div
                ...
                animate={{
                  background: [
                    `radial-gradient(circle at 20% 80%, ${project.color}40, transparent)`,
                    `radial-gradient(circle at 80% 20%, ${project.color}40, transparent)`,
                    `radial-gradient(circle at 20% 80%, ${project.color}40, transparent)`,
                  ],
                }}
                transition={{
              ...
              <div className="relative z-10 p-8 h-full flex flex-col justify-between">
                ...
                <motion.div
                  className="flex items-center text-white group-hover:translate-x-2 transition-transform"
                  initial={{ opacity: 0, x: -20 }}
                  whileInView={{ opacity: 1, x: 0 }}
  ...
  return (
    <section className="py-32 px-6 bg-black relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        >
          <motion.button
            className="relative px-12 py-6 bg-transparent border-2 border-white text-white font-bold text-xl rounded-full overflow-hidden"
            whileTap={{ scale: 0.95 }}
          >
            <motion.div
