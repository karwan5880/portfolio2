...
export default function DevHistoryPage() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      ...
      // Simple debug
      console.log(
        `Scroll: delta=${scrollDelta}, cards=${cardsToMove}, ${currentIndex} → ${scrollingDown ? Math.min(currentIndex + cardsToMove, devHistoryData.length - 1) : Math.max(currentIndex - cardsToMove, 0)}`
      )
    }
    ...
    <div className={styles.page}>
      <div className={styles.container}>
        ...
        <div className={styles.dotsContainer}>
          {devHistoryData.map((_, index) => (
            <button
              ...
              title={devHistoryData[index].title}
            />
          ))}
        </div>
        ...
        <div className={styles.navigation}>
          <button
            onClick={jumpToStart}
            className={`${styles.navButton} ${styles.jumpButton}`}
            disabled={currentIndex === 0}
            aria-label="Jump to first item"
          >
            ⏮ Start
          </button>
          ...
          <button
            onClick={jumpToEnd}
            className={`${styles.navButton} ${styles.jumpButton}`}
            disabled={currentIndex === devHistoryData.length - 1}
            aria-label="Jump to last item"
          >
            End ⏭
          </button>
        </div>
      ...
      <CornerLink href="/design/0/timeline" position="bottom-left" label="Timeline" />
      <CornerLink href="/design/0/skills" position="bottom-right" label="Skills" onNavigateStart={grantPermission} />
    </div>
  )
}
